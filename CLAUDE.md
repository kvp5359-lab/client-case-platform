...
## Перед любой задачей от пользователя или проработкой бага — свериться с баг-логом

🔴 **Правило (по просьбе владельца).** Прежде чем ставить/начинать задачу от пользователя ИЛИ расследовать баг — **сначала проверить [`docs/bugs/`](docs/bugs/)** (индекс в `README.md` + файлы в `open/` и `resolved/`), чтобы понять, **не повторяется ли** проблема и не расследована ли уже. Если похожее есть — продолжать с того места, а не с нуля. Новый баг/повторяющуюся жалобу — фиксировать в `docs/bugs/open/` (один файл = один баг) и обновлять индекс в `README.md`.
