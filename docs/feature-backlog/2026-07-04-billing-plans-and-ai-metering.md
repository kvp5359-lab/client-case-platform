...
## Статус Фаз 3-5 (2026-07-04, по решениям владельца)

Решения владельца: (1) тарифов ~2, состав позже; (2) лимиты **жёсткие** + предупреждение на 95%; (3) токены — **включённая квота**; (4) оплата вручную, потом интеграция; (5) админка нужна.

**Сделано:**
- **Админка** (Фаза 4) ✅ в проде — `platform_admins`, роут `/admin`, назначение тарифов, расход по воркспейсам.
- **Предупреждение 95%** ✅ — баннер `LimitWarningBanner` вверху воркспейса (участники/проекты/хранилище/токены).
- **Квота токенов (жёстко)** 🟡 — `chat-with-messages` при исчерпании отдаёт 402 + понятный текст. Остальные ИИ-функции — по rollout.
- **Лимит проектов (жёстко)** ✅ — гейт в `CreateProjectDialog`.
- **Семантика «участники» = места команды** (participants с `user_id`), НЕ контакты — иначе входящие ломали бы лимит.

**Осталось (Фаза 5):**
- Гейт создания участника команды при лимите (invite-флоу — несколько точек входа).
- Квота токенов в остальных ИИ-функциях (rollout Фазы 2).
- Жёсткость через БД-триггеры — НЕ делать вслепую: `participants` создаётся автоматикой (входящие→контакт), триггер сломает приём. Только per-table анализ user-initiated vs system.
- Оплата (интеграция) — когда решишь.

## Фаза 4 — Платформенная админка (супер-админ) ✅ сделано (см. статус выше)
