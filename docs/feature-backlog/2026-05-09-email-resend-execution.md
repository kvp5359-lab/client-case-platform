...
1. **Замену Postmark → Resend** в инфраструктуре.
2. **Конкретные шаги по фазам** — что делает Claude, что делает пользователь.
3. **Решения по 6 открытым вопросам** v3.
...
В первой итерации только SMTP-отправка (без копирования в папку «Отправленные» сотрудника).
Во второй — добавим IMAP append для синка с Sent.
...
В первой итерации **игнорируем при отправке** (только To). На приёме сохраняем для информации.
Чекбокс «Ответить всем» — отложен.
...
- [ ] Применить миграцию `20260506_email_hybrid_setup.sql` (адаптация: убрать `postmark_*` колонки, добавить `resend_*`).
- [ ] Применить RPC `resolve_inbound_email_address`, `match_inbound_email`, `get_thread_email_address`.
- [ ] Расширить триггер `notify_telegram_on_new_message`.
- [ ] Регенерация TS-типов БД.
...
- [ ] UI «Активировать email» в settings/email.
...
- [ ] **Тест**: настроить forward в личном Gmail на `inbox@demo.clientcase.app` → отправить себе письмо → проверить что попало в нераспознанные (т.к. треда с такой перепиской ещё нет).
...
- [ ] Расширить таблицу `email_accounts`:
  - `auth_type`, `smtp_*`, `imap_*`, `display_name`, `forward_setup_status`, etc.
- [ ] Edge Function `connect-email-account` (Gmail OAuth + SMTP).
- [ ] Refactor `gmail-send` → shared `_shared/sendViaGmail.ts`.
- [ ] `_shared/sendViaSmtp.ts` через `npm:nodemailer`.
- [ ] UI: страница «Мои ящики» в `/profile`.
- [ ] Шифрование SMTP-паролей через pgsodium.

### Phase 5 — Auto-setup forward для Gmail OAuth (1 день) — **Claude автономно**

- [ ] При подключении Gmail OAuth добавить forward через Gmail API.
- [ ] Перехват confirmation code в `resend-webhook` → парсинг кода → confirmation через Gmail API.
...
- [ ] Edge Function с двумя методами:
...
- [ ] Поддержка attachments (Resend API: загрузка через base64 или URL).
- [ ] Reply-threading через `In-Reply-To` / `References`.
- [ ] Frontend invoke в `messengerService` для writing email-сообщений.

### Phase 7 — Bounce webhook (½ дня) — **Claude автономно**

- [ ] Resend bounce/complaint events → обновляют `email_delivery_status`.
- [ ] UI индикатор (как у Wazzup).

### Phase 8 — UI (2-3 дня) — **Claude + Пользователь для верификации**

- [ ] Композер для email-тредов:
...
- [ ] Виртуальные адреса CRUD (опциональная фича).
- [ ] Вкладка «Email» в `IntegrationsTab` воркспейса.

**Чекпоинты пользователя:**
- Проверить что отправка через подключённый Gmail работает (письмо приходит, From правильный).
- Проверить что отправка через `t+15@<slug>` работает (Resend, From с поддомена).
- Проверить что ответ клиента попадает в правильный тред.

### Phase 9 — Активация на rs (после demo) — **совместно**

- [ ] Активировать email для воркспейса `rs.clientcase.app`.
- [ ] Подключить свой Gmail к воркспейсу `rs`.
- [ ] Real-world тест: отправить письмо реальному клиенту, дождаться ответа.

### Phase 10 — Документация (½ дня) — **Claude автономно**

- [ ] Обновить `infrastructure.md` — раздел про email.
- [ ] Закрыть `2026-05-04-email-postmark-internal-addresses.md` как «реализовано через Resend».
...
1. **DNS-провайдер для clientcase.app** — Hostinger или Cloudflare?
...
2. **Что с домен-репутацией** на старте?
...
3. **«Marketing emails» план Resend** — игнорируем?
