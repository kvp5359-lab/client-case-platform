...
- **T4** ✅ — мёртвый `taskKeys.urgentCount`-кластер удалён (+6 инвалидаций, тест обновлён на живой `my-task-counts`); дубль-ключ `templateIdForProject` → `projectTemplateKeys.idByProject`; accent-карты под защиту типов: 3 не-карантинных `Record<ThreadAccentColor>` (UnreadBadge, TimelineFeed, InboxChatHeader); 2 карантинных `Record<MessengerAccent>` (ReactionBadges, MessageInputToolbar — у них ЕСТЬ все 12 значений вкл. green/dark, индекс без каста) — добавлено в отложенном-проходе 2026-06-13. `threadConstants.COLOR_TEXT` (10 ключей, 7 free-string индекс-сайтов в не-карантинных файлах) НЕ трогал — каст в 7 местах ради одной из двух карт (вторая генерится через fromEntries), churn > выгода.
- **T1** — ⏳ частично:
