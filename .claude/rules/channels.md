...
**v1 webhook (`telegram-webhook`)** — формально «legacy» после миграции 2026-05-28, но **НЕ мёртвый**. ⚠️ Аудит 2026-06-13 (`getWebhookInfo` по всем ботам): v1 держится на **ОДНОМ** боте — `rs_help103_bot` (employee), который обслуживает **живой** клиентский тред «Клиенты» (сообщения ежедневно). Остальные 4 бота уже на v2. Плюс `telegram-register-webhook:109` ставит URL **новых** ботов на **v1**. Удаление v1 сломает приём у rs_help103. **НЕ удалять `telegram-webhook` и НЕ дропать `bot_version`** до миграции rs_help103 на v2 (нужны ДВЕ скоординированные правки: `setWebhook`→v2 + `UPDATE project_telegram_chats.bot_version='v2'` его чатов, иначе v2-фильтр `bot_version='v2'` не найдёт биндинг). Полный план миграции + откат — F1 в [`docs/audit/2026-06-13-quarantine-audit.md`](../../docs/audit/2026-06-13-quarantine-audit.md).
