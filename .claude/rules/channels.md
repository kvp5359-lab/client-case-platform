...
**v1 webhook (`telegram-webhook`) — УДАЛЁН 2026-07-12.** Ретайр после авторитетной проверки `getWebhookInfo` по всем 5 активным ботам: реальный webhook у ВСЕХ = v2 (включая секретаря `f87dba85`, у которого `config.bot_version` дрейфанул на 'v1', но по факту v2 — 55 активных v2-привязок, свежая активность). Единственная v1-привязка была неактивной на мёртвом треде «Команда» (март) — флипнута в v2 при ретайре. Удалены: edge `telegram-webhook` + `telegram-setup-webhook` (одноразовый регистратор v1, ставил legacy env-токен), их исходники, записи в `deploy-edge.sh`/`config.toml`. `telegram-register-webhook` ставит новых ботов на **v2**. При онбординге новых ботов v1 больше не существует. (История F1 — [`docs/audit/2026-06-13-quarantine-audit.md`](../../docs/audit/2026-06-13-quarantine-audit.md).)
