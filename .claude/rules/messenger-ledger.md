...
| 2026-06-09 | **Точка отсчёта непрочитанного = момент выдачи доступа** (вместо «от начала времён»). Сейчас новый участник получает тысячи фантомных непрочитанных | 🔜 **Согласовано, не начато.** План: формула в `get_inbox_threads_v2` + сидирование `message_read_status` при выдаче доступа (`added_at`/`assigned_at`), старое — обнулить разово. Затрагивает 8 RPC-мест (см. журнал 2026-06-09) |
