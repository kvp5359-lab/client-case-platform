...
- **Проверки:** tsc 0, lint 0, **707 тестов**. **⏳ Ждёт деплоя + смок:** у задачи с непрочитанным на 2-й странице инбокса бейдж виден в списке и на вкладке; при открытии/прочтении — сбрасывается.
- **Файлы:** `UnreadBadge.tsx`, `TaskPanelTabbedShellRenderer.tsx`. Источник правды бейджа — `inboxKeys.aggregates` (полный), не `inboxKeys.threads` (пагинированный).
