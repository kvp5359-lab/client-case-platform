...
| 2026-06-13 | **Вкладка «Ждут ответа» + тост «Новый диалог»** (CRM follow-up). RPC `get_inbox_awaiting_reply_threads` в проде, фронт готов (706 тестов), тост на `project_threads` INSERT в `useNewMessageToast` | ⏳ **Ждёт деплоя фронта + смок:** написал клиенту первым в TG/WhatsApp → тост «Новый диалог» + диалог во вкладке «Ждут ответа»; клиент ответил → ушёл в «Непрочитанные». Детали — журнал ниже |
