...
Реализовано 2026-07-04; 2026-07-16 переписано на **модель Планфикса**: колонки — ядро, группировка вешается на поле колонки, отдельных «показателей» и режимов «сводка/список» НЕТ.

**Колонка** = `{ key (поле датасета), label? (свой заголовок), agg?, align? }`, где `agg` — «при группировке выводить»: `none|count|sum|avg|min|max` (sum/avg/min/max — только числовые поля). Поэтому суммировать можно **любую** числовую колонку, а не только заранее описанный показатель. Ключ поля может повторяться (две колонки по `amount`: «Сумма» и «Средний чек»).

**Вид следует из настроек:**

| Настройки | Что видно |
|-----------|-----------|
| `groupBy=[]`, `showRecords=true` | плоский список записей |
| `groupBy=[]`, `showRecords=false` | одна строка итогов |
| `groupBy=[…]`, `showRecords=false` | только строки групп с агрегатами |
| `groupBy=[…]`, `showRecords=true` | дерево: группы, внутри записи по клику |

**Одна сетка колонок на весь отчёт** (одна шапка): строка группы = значение уровня **в колонке этого уровня** + агрегаты колонок; строка записи = свои значения во всех колонках; «Итого» = те же агрегаты по всему набору (автоматически, по всем колонкам с `agg`). Пустых колонок не возникает именно потому, что группировка живёт в своей колонке — это была структурная проблема первой версии.
...
- **Legacy вычищен полностью** (`mode`/`measures`/`columns: string[]`): на момент переписывания в базе было 2 отчёта, их конфиги переложены миграцией. Поддержки старого формата ни на фронте, ни в RPC больше нет.
- **RPC `run_report(workspace_id, config)`** — движок ([`20260716120000_reports_columns_model.sql`](../../supabase/migrations/20260716120000_reports_columns_model.sql); первая версия — [`20260704130100_run_report_engine.sql`](../../supabase/migrations/20260704130100_run_report_engine.sql)). SECURITY INVOKER (RLS нижележащих таблиц применяется к вызывающему). Клиент передаёт только КЛЮЧИ — датасеты/поля/агрегаты/операторы резолвятся через **whitelist-реестр**, значения фильтров и `recordsFor` через `quote_literal`. Сырой SQL от клиента невозможен.
- **Датасеты (v1)**: `transactions` (движение денег), `services` (выставлено), `client_balance` (выставлено − оплачено = долг), `projects`, `threads` (по RLS-видимости вызывающего).
- **Финансы в `projects`** (2026-07-16): поля `billed` (сумма услуг), `paid` (доходы), `expenses` (расходы), `balance` (долг) — считаются **LATERAL-подзапросами** по каждому проекту. 🪤 Обычным JOIN к `project_services`/`project_transactions` их брать НЕЛЬЗЯ: строки проекта размножатся и суммы задвоятся. Сверено с датасетом `transactions` — доходы/расходы совпадают до копейки. Отсюда же и разница с `client_balance`: тот датасет — про долг (одна строка на проект), `projects` умеет ещё дедлайн и быстрый период по дате создания.
- **⚠️ Два реестра — держать в синхроне**: серверный — функция **`_report_registry()`** (вынесен из тела `run_report` 2026-07-16: раньше каждая правка функции требовала копировать 105 строк JSON) и клиентский [`src/lib/reports/registry.ts`](../../src/lib/reports/registry.ts) (лейблы, типы для UI, форматы, дефолтные конфиги). Новое поле/датасет = правка обоих.
...
- **Сортировка** — `sort: { by: 'cN', dir }`, где `cN` = **индекс колонки** в `columns`. Одно поле задаёт и порядок записей (сервер, `ORDER BY` по выражению поля; без `sort` — `detail_order` датасета), и порядок групп (клиент, `sortNodes`: по агрегату колонки, а если его нет — по названию группы). 🪤 Ссылка по индексу хрупкая: при перестановке/удалении колонок `ReportSettingsDialog` переносит `sort` за колонкой по identity объекта либо сбрасывает — правка атрибутов колонки (`patchColumn`) порядок не меняет и `sort` не трогает.
- **Период** не хранится в отчёте — пресет резолвится в даты на каждый запуск (`applyPeriodToConfig` в [`src/lib/reports/runtime.ts`](../../src/lib/reports/runtime.ts)) → «последние 30 дней» скользящие. Набор пресетов один (`PERIOD_PRESET_OPTIONS`): страница отчёта + оператор «период» в фильтре.
- **🪤 Оператор `dyn_period` — клиентский**: в фильтре хранится пресет («этот месяц»), RPC его НЕ понимает. Любой вызов `run_report` обязан сперва прогнать конфиг через `resolveDynamicPeriods` (пресет → `between` с датами) — страница делает это при сборке runtimeConfig, догрузка записей и CSV получают уже резолвленный.
- **Период фильтра = дефолт быстрого выбора, а не второе условие.** Страница стартует с периода из фильтра (`extractPeriodFromConfig`: `dyn_period` → сам пресет, `between` → «Период…» с датами), но перед запуском ВЫРЕЗАЕТ периодные условия по `periodField` из фильтра (`stripPeriodConditions`) и применяет только выбранный на странице период. Иначе «этот месяц» из фильтра ∧ «прошлый месяц» со страницы = пусто. Условия по другим полям-датам (напр. дедлайн) не трогаются.
- **Результат**: строка группы → `g0..gN` (значения уровней) + `c0..cM` (агрегаты, **индекс = позиция колонки в `columns`**) + `level`; строка записи → те же `c0..cM`, но со своими значениями. Уровни считает Postgres через `GROUP BY GROUPING SETS` по префиксам групп, поэтому **агрегаты на всех уровнях честные** (sum/avg/count — по всем данным группы, а не по догруженным записям). Клиент только собирает дерево (`buildReportTree`) и сортирует узлы.
- **🪤 `level` обязателен**: строка подытога и реальная группа с пустым значением обе дают `gN=null` (например, «Чат» без срока: `level=1` — подытог, `level=2` — группа «нет срока»). Различает их только `level`, который выводится из `GROUPING()`. Не определять уровень по «сколько gN не-null».
- **Записи внутри группы** — отдельный вызов `run_report` с `recordsFor` (значения групп по уровням), монтируется лениво при раскрытии → LIMIT 500 действует на группу, а не на отчёт. Отбор идёт по ТЕМ ЖЕ выражениям, что и группировка, поэтому состав записей всегда сходится с подытогом (в т.ч. у тёзок, которых группа схлопывает по имени).
- **Ссылки на проект/тред в записях**: у поля в реестре БД есть `link` (`project`/`thread`) + `link_id` (+ `link_project` у треда) — сервер отдаёт id доп. ключами `cN_id`/`cN_pid`, фронт рисует настоящий `<a href>` (нужно для открытия средней кнопкой). URL строит [`src/lib/entityLinks.ts`](../../src/lib/entityLinks.ts) — общий с глобальным поиском. 🪤 У треда своей страницы нет: ссылка = страница проекта с `?panelTab=thread:<id>`, а для тредов без проекта — `/inbox` (панель восстанавливается через `useThreadFromPanelTab`).
- **Лимиты**: записи 500, строки с группами 1000 (`limitHit` в ответе). CSV выгружает листовой уровень дерева (записи в него не идут — они лениво догружаются).
