...
`react-hook-form` и `zod` **удалены из зависимостей** (были историческим легаси от shadcn-init, в реальных формах не использовались). Формы — на чистом контролируемом React. При написании новой формы не подтягивать эти библиотеки.
