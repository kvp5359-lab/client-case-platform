...
1. Перед изменениями — прочитать [`channels.md`](./channels.md) (соответствующий раздел канала) + [`gotchas.md`](./gotchas.md) (RLS short-circuit, multi-bot dedup, секреты).
...
**Что проверяем:**
- Все query keys — в `src/hooks/queryKeys/` (директория модулей + barrel, единый источник)
