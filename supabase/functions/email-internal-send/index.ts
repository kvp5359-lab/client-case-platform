 ...
 * Вынесено из тела Deno.serve (распил #3 аудита 2026-06-13) — поведение
 * не менялось, только структура: две ветки (Gmail/Resend) стали функциями.
 */
interface OutboundEmailCtx {
...
// =============================================================
// Каналы отправки (вынесены из Deno.serve, распил #3 — логика не менялась)
// =============================================================
...
async function sendViaResend(ctx: OutboundEmailCtx, w: WorkspaceRow): Promise<Response> {
  const { service, m, t, req, senderName, subjectRaw, subjectRoot, inReplyTo, references, html, text, attachments } = ctx;
