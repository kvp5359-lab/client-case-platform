...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (update.message_reaction_count) {
      const { syncTelegramReactionsAggregated } = await import(
        "../_shared/syncTelegramReactions.ts"
      );
      await syncTelegramReactionsAggregated(serviceClient, update.message_reaction_count);
      return new Response("ok", { status: 200 });
    }
