-- get_workspace_threads: добавлен флаг email_unsent для метки «Черновик» в
-- списках/досках. true только для type='email' без отправленных сообщений
-- (нет project_messages с is_draft=false). Смена RETURNS TABLE → DROP+CREATE.
--
-- ⚠️ Тело снято с ЖИВОЙ функции в проде (RPC в состоянии drift, см. ledger) и
-- дополнено колонкой. После DROP+CREATE Supabase возвращает PUBLIC/anon в гранты
-- по умолчанию → REVOKE + явный GRANT (исходно: authenticated, service_role).

DROP FUNCTION IF EXISTS public.get_workspace_threads(uuid, uuid);
...
REVOKE EXECUTE ON FUNCTION public.get_workspace_threads(uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_workspace_threads(uuid, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_workspace_threads(uuid, uuid) TO authenticated, service_role;
