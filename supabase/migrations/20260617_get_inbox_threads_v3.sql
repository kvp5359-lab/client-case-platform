-- Фаза 2.5: v3 читает из материализованных thread_unread_state + thread_inbox_meta.
-- Display-поля резолвятся LATERAL'ами только для строк результата. Байт-идентичен v2
-- (сверено 0/0 на 7 юзерах, кроме недетерминированных ничьих). Выигрыш при пагинации
-- (574мс → ~10мс). project_name пер-юзер: gate доступом к проекту (как accessible_projects).
...
$FN$;
REVOKE ALL ON FUNCTION public.get_inbox_threads_v3(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_inbox_threads_v3(uuid, uuid) TO authenticated, service_role;
