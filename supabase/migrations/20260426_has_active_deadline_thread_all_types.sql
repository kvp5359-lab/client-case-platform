-- Расширяем поле has_active_deadline_task в get_accessible_projects:
-- теперь учитываются все типы тредов (task / chat / email), а не только task.
-- Условие: тред не удалён, имеет deadline, и его статус не финальный
-- (или статус не задан вовсе).
--
-- Поле в RETURNS TABLE намеренно оставлено с прежним именем
-- has_active_deadline_task ради совместимости с типами и фронтендом.
