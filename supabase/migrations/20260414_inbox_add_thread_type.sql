-- Добавляем поле thread_type в RPC get_inbox_threads_v2.
--
-- Было: BoardInboxList.threadToTaskItem жёстко ставил type: 'chat' для всех
-- открываемых из инбокса тредов, потому что v2 не отдавала тип. В результате
-- при клике на задачу в инбокс-колонке боковая панель получала снимок с
-- type = 'chat', и дропдаун статуса считал её не-задачей (currentStatus = null).
--
-- TaskPanel теперь мержит актуальный тип из useProjectThreadById, но это
-- устраняет симптом с задержкой. Здесь чиним корень: RPC отдаёт thread_type,
-- BoardInboxList передаёт его как есть — снимок сразу правильный.
