-- get_inbox_threads_v2: имя/аватар собеседника email-треда резолвится по email.
-- Раньше это был обычный LEFT JOIN participants по lower(email) — без LIMIT 1.
-- email НЕ уникален в participants → при двух контактах с одним адресом каждая
-- строка email-треда дублировалась (фан-аут). Один thread_id на двух строках →
-- в UI «Входящих» обе подсвечивались как один тред + коллизия React-key.
-- Фикс: LEFT JOIN LATERAL (... ORDER BY created_at LIMIT 1) — максимум 1 контакт на email.
-- Защита от повтора на уровне данных — uq_participants_workspace_email_active.
-- Сигнатура не меняется → гранты сохраняются (CREATE OR REPLACE).
-- См. messenger-ledger 2026-06-17.
...
AS $function$
  ...
  -- email-собеседник: максимум 1 контакт на адрес (фикс фан-аута 2026-06-17)
  LEFT JOIN LATERAL (
