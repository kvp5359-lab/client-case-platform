-- smoke_send_file: +p_dispatch (default true). Для TG-группы/Wazzup force-dispatch
-- шлёт вложения; для MTProto/email раннер зовёт edge как смок-бот (p_dispatch=false).
DROP FUNCTION IF EXISTS public.smoke_send_file(uuid, uuid, boolean, jsonb);
CREATE OR REPLACE FUNCTION public.smoke_send_file(
  p_thread_id uuid, p_message_id uuid, p_with_text boolean, p_files jsonb, p_dispatch boolean DEFAULT true
)
RETURNS uuid
...
BEGIN
  ...
  END LOOP;
  IF p_dispatch THEN PERFORM dispatch_message_to_channels(p_message_id, true); END IF;
  RETURN p_message_id;
END;
$$;
REVOKE ALL ON FUNCTION public.smoke_send_file(uuid, uuid, boolean, jsonb, boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.smoke_send_file(uuid, uuid, boolean, jsonb, boolean) TO service_role;
