-- Вкладка «Документы» в пикере «молнии»: у слота появился признак has_document —
-- документ загружен. Фронт по нему предлагает три режима вставки: показывать
-- загруженные как обычно, зачёркивать или скрывать (остаётся «что осталось
-- прислать»).
--
-- «Загружен» = document_id заполнен И документ не в корзине — зеркалит клиентскую
-- логику вкладки «Документы» (useFolderSlots подменяет удалённый документ на
-- пустой слот).
--
-- Остальное тело — без изменений относительно 20260715210000_shareable_doc_tree.
