-- Риск-оценка полей анкеты (🟢🟡🔴).
--
-- 1. Флаг "поле поддерживает риск-оценку" — точечно на уровне записи поля в шаблоне
--    (form_template_fields), не на field_definitions: одно и то же поле в разных
--    анкетах может требовать или не требовать оценку. Флаг копируется в form_kit_fields.
-- 2. Сама оценка (green/yellow/red) живёт рядом с ответом в form_kit_field_values,
--    чтобы переживать пересинхронизацию структуры анкеты с шаблоном.

ALTER TABLE public.form_template_fields
  ADD COLUMN IF NOT EXISTS risk_assessment_enabled boolean NOT NULL DEFAULT false;

ALTER TABLE public.form_kit_fields
  ADD COLUMN IF NOT EXISTS risk_assessment_enabled boolean NOT NULL DEFAULT false;

ALTER TABLE public.form_kit_field_values
  ADD COLUMN IF NOT EXISTS risk_level text
  CHECK (risk_level IS NULL OR risk_level IN ('green', 'yellow', 'red'));

-- Прокидываем флаг при создании анкеты из шаблона.
-- Тело взято из 20260524_section_header_color.sql + risk_assessment_enabled рядом с is_required.
...
-- Прокидываем флаг при пересинхронизации структуры.
-- Значения (включая risk_level) сохраняются — DELETE затрагивает только form_kit_fields/sections,
-- а form_kit_field_values остаётся и переиспользуется по (form_kit_id, field_definition_id).
