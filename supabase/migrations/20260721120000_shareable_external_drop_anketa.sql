-- Убрать «Анкету» (form_kits.google_sheet_id) из вкладки «Внешние» пикера молнии.
-- В рабочем процессе набор всегда генерит только бриф (brief_sheet_id); ветка
-- google_sheet_id заполнена лишь у пары старых записей и путала. Данные не трогаем,
-- только перестаём показывать. Остальные ветки (папка проекта, папки набора,
-- подпапки, бриф) без изменений.
