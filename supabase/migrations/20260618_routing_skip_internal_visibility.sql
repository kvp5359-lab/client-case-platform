-- Фаза 2 (КАРАНТИН): skip внешней доставки для внутренних сообщений.
-- dispatch_message_to_channels получает guard: visibility <> 'client' → наружу
-- НЕ отправляем, помечаем send_status='sent' (внутреннее доставлено в ЛК).
--
-- ⚠️ Карантин (путь отправки). Тело снято с прода ДОСЛОВНО + добавлен ОДИН guard
--    после проверки пустого контента, до всех веток отправки. No-op для текущих
--    сообщений (все visibility='client'). Требует живого смок-теста ВСЕХ каналов:
--    обычный ответ клиенту по-прежнему доходит; team/self наружу НЕ уходят.
...
BEGIN
  ...
  -- ВНУТРЕННЕЕ СООБЩЕНИЕ (не клиенту): наружу не отправляем, помечаем доставленным.
