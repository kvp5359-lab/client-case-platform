-- Шаблон треда несёт дефолты для создаваемого треда:
--   default_project_id  — проект, в котором сразу заводится тред (там, где нет
--                         контекста проекта: глобальное меню «+/Новый»).
--                         ON DELETE SET NULL — удалили проект, шаблон живёт дальше.
--   default_description — описание по умолчанию (внутренняя заметка команды),
--                         подставляется в project_threads.description. Отдельно
--                         от thread_templates.description (та описывает сам шаблон).
...
-- RPC обновления шаблона (whitelist колонок) должна уметь писать новые поля.
