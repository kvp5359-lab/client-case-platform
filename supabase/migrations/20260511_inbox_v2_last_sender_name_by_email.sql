-- get_inbox_threads_v2: last_sender_name теперь тоже резолвится через
-- participants по email (для входящих email с sender_participant_id=NULL).
-- JOIN email_counter_p сделан безусловным — guard по sender_p/counter_p
-- мешал когда participant найден для одного, но не для другого.
--
-- Полное определение функции — см. предыдущую миграцию
-- 20260511_inbox_v2_counterpart_by_email.sql. Здесь только diff в SELECT:
--   * last_sender_name: COALESCE(sender_p.name+last_name, email_counter_p.name+last_name, lm.sender_name)
--   * last_sender_avatar_url: COALESCE(sender_p.avatar_url, email_counter_p.avatar_url)
--   * JOIN email_counter_p — безусловный (без guard на sender_p.id IS NULL)
