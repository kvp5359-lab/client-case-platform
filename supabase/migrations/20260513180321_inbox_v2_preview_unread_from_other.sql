-- Фикс превью треда в инбоксе: если у меня есть непрочитанные сообщения
-- от собеседника, в превью показывать последнее из НИХ, а не моё исходящее.
--
-- Сценарий: после непрочитанного сообщения клиента я отправляю ответ
-- (например, через MTProto в Telegram). Тред продолжает быть в "Входящих"
-- (бейдж непрочитанных правильный), но в превью видно МОЁ последнее
-- сообщение, и непонятно, на что отвечать. Должно быть видно сообщение
-- клиента, которое заставило тред быть непрочитанным.
--
-- Изменение точечное — переписан CTE `last_messages`: при выборе DISTINCT ON
-- (thread_id) сначала по приоритету «непрочитанное от собеседника», потом по
-- created_at DESC. Если непрочитанных от собеседника нет — поведение прежнее
-- (последнее сообщение треда). Остальные CTE/JOIN'ы и сортировка не тронуты.
...
AS $function$
  ...
  last_messages AS (
    -- Превью треда: если у меня есть непрочитанные сообщения от собеседника,
    -- показываем последнее из НИХ; иначе — последнее сообщение треда.
  ...
  SELECT
    ...
    at.legacy_channel::TEXT, lm.message_at, lm.message_text::TEXT, lma.first_file_name::TEXT, COALESCE(lma.file_count, 0),
    COALESCE(
