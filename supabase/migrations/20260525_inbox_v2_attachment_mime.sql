-- get_inbox_threads_v2: добавляем last_message_attachment_mime (mime_type первого
-- вложения последнего сообщения). Нужно фронту для осмысленного превью во
-- «Входящих», когда content — плейсхолдер «📎» или пустой:
...
--   default   → «📎 имя файла»
--
-- CREATE OR REPLACE не работает при изменении RETURNS TABLE signature —
-- сначала DROP. Остальное тело CTE идентично предыдущей версии RPC, добавлен
-- only `first_mime_type` в last_message_attachments + одно поле в SELECT.
...
RETURNS TABLE(
  thread_id uuid, thread_name text, thread_icon text, thread_accent_color text, thread_type text,
  project_id uuid, project_name text,
  channel_type text, legacy_channel text,
  last_message_at timestamp with time zone, last_message_text text,
  last_message_attachment_name text, last_message_attachment_count integer,
  last_message_attachment_mime text,
  last_sender_name text, last_sender_avatar_url text,
  unread_count bigint, manually_unread boolean,
  has_unread_reaction boolean, unread_reaction_count bigint,
  last_reaction_emoji text, last_reaction_at timestamp with time zone,
  last_reaction_sender_name text, last_reaction_sender_avatar_url text,
  last_reaction_message_preview text,
  email_contact text, email_subject text,
  last_event_at timestamp with time zone, last_event_text text, last_event_status_color text,
  unread_event_count bigint,
  counterpart_name text, counterpart_avatar_url text,
