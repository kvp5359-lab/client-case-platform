-- ============================================================================
-- get_inbox_threads_v2: orphan-треды (project_id IS NULL) видны не только
-- владельцу, но и исполнителям (task_assignees) и custom-members
-- (project_thread_members).
--
-- Контекст: миграция 20260520_orphan_thread_access_for_any_type.sql
-- открыла доступ через RLS-функцию can_user_access_thread, но RPC
-- get_inbox_threads_v2 имеет собственный CTE accessible_threads с жёстко
-- зашитой проверкой owner_user_id = p_user_id для orphan-веток. В
-- результате RLS пропускает Анну к треду, но RPC её не возвращает в
-- списке инбокса.
--
-- Что меняем: расширяем orphan-ветку accessible_threads, добавляя в OR
-- условия:
--   - текущий юзер в task_assignees этого треда;
--   - текущий юзер в project_thread_members этого треда.
-- ============================================================================
...
AS $function$
  ...
  accessible_threads AS (
    ...
    UNION
    -- Orphan-треды (project_id IS NULL): владелец, исполнитель или явный
    -- custom-member.
    SELECT pt.id, pt.project_id, pt.name, pt.icon, pt.accent_color, pt.legacy_channel, pt.access_type, pt.type,
           pt.business_client_tg_user_id, pt.mtproto_client_tg_user_id, pt.wazzup_contact_avatar_url,
           pt.email_last_external_address
