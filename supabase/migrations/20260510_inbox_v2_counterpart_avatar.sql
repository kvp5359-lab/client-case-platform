-- Расширяет get_inbox_threads_v2 двумя полями: counterpart_name и
-- counterpart_avatar_url. counterpart = автор последнего сообщения от
-- НЕ-сотрудника (sender_role IS NULL или NOT IN team-роли). Аватар
-- приходит из множества источников:
--   1) participants.avatar_url (если у клиента есть participant)
--   2) telegram_user_avatars по business/mtproto/group tg_user_id
--   3) project_threads.wazzup_contact_avatar_url
-- Если ни одного — NULL, UI рисует инициал по counterpart_name.
...
CREATE OR REPLACE FUNCTION public.get_inbox_threads_v2(p_workspace_id uuid, p_user_id uuid)
 RETURNS TABLE(thread_id uuid, thread_name text, thread_icon text, thread_accent_color text, thread_type text, project_id uuid, project_name text, channel_type text, legacy_channel text, last_message_at timestamp with time zone, last_message_text text, last_message_attachment_name text, last_message_attachment_count integer, last_sender_name text, last_sender_avatar_url text, unread_count bigint, manually_unread boolean, has_unread_reaction boolean, unread_reaction_count bigint, last_reaction_emoji text, last_reaction_at timestamp with time zone, last_reaction_sender_name text, last_reaction_sender_avatar_url text, last_reaction_message_preview text, email_contact text, email_subject text, last_event_at timestamp with time zone, last_event_text text, last_event_status_color text, unread_event_count bigint, counterpart_name text, counterpart_avatar_url text)
 LANGUAGE sql
...
AS $function$
  ...
  last_client_messages AS (
    SELECT DISTINCT ON (pm.thread_id)
      ...
      pm.sender_name,
      pm.sender_participant_id,
      pm.telegram_sender_user_id,
      pm.source
    FROM project_messages pm
    INNER JOIN accessible_threads at ON at.id = pm.thread_id
    WHERE pm.source != 'telegram_service'::message_source
      AND (
        pm.sender_role IS NULL
        OR pm.sender_role NOT IN ('Администратор','Владелец','Сотрудник','Исполнитель')
      )
    ORDER BY pm.thread_id, pm.created_at DESC
  ),
  ...
  SELECT
    ...
    COALESCE(ua.cnt, 0),
    COALESCE(counter_p.name, lcm.sender_name)::TEXT,
    COALESCE(
      counter_p.avatar_url,
      tg_av_business.avatar_url,
      tg_av_mtproto.avatar_url,
      tg_av_group.avatar_url,
      at.wazzup_contact_avatar_url
    )::TEXT
  ...
  LEFT JOIN telegram_user_avatars tg_av_business
    ON at.business_client_tg_user_id IS NOT NULL
   AND tg_av_business.tg_user_id = at.business_client_tg_user_id
   AND tg_av_business.is_missing = FALSE
  LEFT JOIN telegram_user_avatars tg_av_mtproto
    ON at.mtproto_client_tg_user_id IS NOT NULL
   AND tg_av_mtproto.tg_user_id = at.mtproto_client_tg_user_id
   AND tg_av_mtproto.is_missing = FALSE
  LEFT JOIN telegram_user_avatars tg_av_group
    ON lcm.telegram_sender_user_id IS NOT NULL
   AND tg_av_group.tg_user_id = lcm.telegram_sender_user_id
   AND tg_av_group.is_missing = FALSE
