-- Repo↔prod parity fix. get_inbox_threads_v3_for was last patched IN PROD ONLY
-- (2026-07-07, "exclude soft-deleted threads" — done via pg_get_functiondef+replace,
-- migration 20260707120000 recreated only get_inbox_thread_aggregates, NOT v3_for).
-- The latest FULL body of v3_for in the repo was 20260701_inbox_preview_skip_deadline_silent.sql,
-- which lacked `JOIN project_threads ... is_deleted = false`. A clean rebuild from repo
-- would resurrect the "deleted thread stuck in Inbox" bug.
-- This migration re-asserts the CURRENT production body (snapshot 2026-07-10) so a fresh
-- `db push` produces the same function prod runs. Idempotent CREATE OR REPLACE; signature
-- unchanged; grants preserved. No prod change (prod already runs this exact body).
--
-- INVARIANTS (do not drop when editing — each was a separate fix):
--   * JOIN project_threads pt ... AND pt.is_deleted = false        (2026-07-07 excl. deleted)
--   * event LATERAL: AND al.action <> 'change_deadline'            (deadline not an event)
--   * event LATERAL: silent_transition guard on change_status
--   * project name gated per-user (can_view_all OR project_participant)
--   * email counterpart via LATERAL ... LIMIT 1 (no fan-out on duplicate email)
