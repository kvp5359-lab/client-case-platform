-- get_inbox_threads_v2: для email-тредов резолвим counterpart_name по email-адресу.
--
-- Раньше у входящих email-сообщений sender_participant_id всегда NULL,
-- и counterpart_name брался как fallback из sender_name (= email отправителя).
-- После переименования контакта в карточке имя в списке тредов не менялось.
--
-- Теперь дополнительно JOIN'им participants по email из двух источников:
--   - project_thread_email_links.contact_email (для проектных email-тредов)
--   - project_threads.email_last_external_address (для личных диалогов без проекта)
-- И отдаём counterpart_name как имя + фамилия participant'а, если найден.
...
AS $function$
  ...
  accessible_threads AS (
    ...
    SELECT pt.id, pt.project_id, pt.name, pt.icon, pt.accent_color, pt.legacy_channel, pt.access_type, pt.type,
           ...
           pt.email_last_external_address
    FROM project_threads pt
    WHERE pt.workspace_id = p_workspace_id AND pt.project_id IS NULL AND pt.is_deleted = false
  ),
  last_messages AS (
  ...
  SELECT
    ...
    lm.sender_name::TEXT, sender_p.avatar_url::TEXT, COALESCE(uc.cnt, 0), COALESCE(mu.manually_unread, FALSE),
  ...
  LEFT JOIN participants email_counter_p
    ON counter_p.id IS NULL
   AND email_counter_p.workspace_id = p_workspace_id
