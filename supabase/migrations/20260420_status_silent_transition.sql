-- Флаг «тихого перехода»: при смене статуса задачи на статус с silent_transition=true
-- audit-событие всё равно записывается (для истории в ленте чата), но не учитывается
-- в unread_event_count — у участников не появляется бейдж непрочитанности.

ALTER TABLE public.statuses
  ADD COLUMN IF NOT EXISTS silent_transition boolean NOT NULL DEFAULT false;

-- Обновляем RPC get_inbox_threads_v2: исключаем из unread_audit переходы
-- в статусы с silent_transition=true.
...
AS $function$
  ...
  unread_audit AS (
    -- Исключаем переходы в статусы с silent_transition=true: такие события
    -- остаются в истории (last_audit), но не инкрементируют бейдж непрочитанных.
