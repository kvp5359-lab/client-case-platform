-- Смок-отправка с опциональным reply (цитата) + метка комбинации. Дроп старой
-- одноаргументной версии (иначе неоднозначный вызов). Применено в прод.
DROP FUNCTION IF EXISTS public.smoke_send_test(uuid);
CREATE OR REPLACE FUNCTION public.smoke_send_test(
  p_thread_id uuid, p_reply_to uuid DEFAULT NULL, p_label text DEFAULT NULL
)
RETURNS uuid
...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'reply-оригинал % не в треде', p_reply_to; END IF;
  ...
  VALUES (p_thread_id, v_ws, COALESCE(NULLIF(v_name,''),'SMOKE-TEST'), v_pid, 'Владелец', v_body, 'web', 'client', p_reply_to)
  RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;
REVOKE ALL ON FUNCTION public.smoke_send_test(uuid, uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.smoke_send_test(uuid, uuid, text) TO service_role;
