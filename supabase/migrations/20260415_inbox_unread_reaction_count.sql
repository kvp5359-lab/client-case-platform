-- Добавляем unread_reaction_count в RPC get_inbox_threads_v2.
--
-- Было: RPC отдавал только булев has_unread_reaction и last_reaction_emoji
-- (последней реакции). Фронт не мог отличить 1 непрочитанную реакцию от 5 —
-- в бейдж засчитывал всегда +1. Теперь возвращаем фактическое число
-- непрочитанных реакций в треде, чтобы при 2+ показывать цифру, а при 1 —
-- по-прежнему эмодзи.
--
-- has_unread_reaction и last_reaction_emoji сохранены: они используются для
-- превью «Alice отреагировал(а) 👍 на: ...» в InboxChatItem.
...
AS $function$
  ...
  unread_reaction_counts AS (
    -- Сколько НЕПРОЧИТАННЫХ чужих реакций в треде после last_read_at пользователя.
    -- Тот же фильтр, что для has_unread_reaction: mr.participant_id != self,
    -- и реакция новее last_read_at (либо last_read_at отсутствует).
    SELECT pm.thread_id, COUNT(*) AS cnt
    FROM message_reactions mr
    INNER JOIN project_messages pm ON pm.id = mr.message_id
    INNER JOIN accessible_threads at ON at.id = pm.thread_id
