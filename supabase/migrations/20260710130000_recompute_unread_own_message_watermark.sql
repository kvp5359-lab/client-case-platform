-- Фикс гонки счётчика непрочитанного: отправка своего сообщения оставляла
-- застрявший unread_count. Когда участник шлёт сообщение, одновременно (1) INSERT
-- сообщения триггерит recompute_thread_unread_for и (2) markAsRead двигает
-- last_read_at. recompute от INSERT успевает прочитать СТАРЫЙ last_read_at (до
-- markAsRead) → считает прежние входящие непрочитанными и коммитит ПОСЛЕ markAsRead
-- → перетирает правильный 0. В UI: чат помечен непрочитанным, но у баблов нет
-- контуров непрочитанности (contour берёт last_read_at из message_read_status,
-- который уже верный).
--
-- Фикс: водяной знак «своё последнее сообщение» для СЧЁТЧИКОВ. Отправив сообщение,
-- участник неявно прочитал всё до него. AFTER-INSERT-триггер видит только что
-- вставленное своё сообщение (v_own_last) и поднимает границу v_wm =
-- GREATEST(last_read_at, own_last) — обе ветки гонки дают одинаковый результат.
-- Хранимый last_read_at НЕ трогаем (его читают inbox-сортировки). GREATEST
-- игнорирует NULL и только повышает границу «прочитано» → лишних непрочитанных
-- не создаёт.
--
-- ⚠️ DRIFT: тело функции в проде расходилось с репо (обновлялось напрямую через
-- MCP). Здесь — полное актуальное тело прода + водяной знак. Применено в прод
-- 2026-07-10 через MCP; этот файл — для воспроизводимости (fresh db push).
