-- Фикс утечки личных диалогов через инбокс.
-- Раньше второй UNION-блок в accessible_threads добавлял ВСЕ треды с
-- project_id IS NULL независимо от owner_user_id. Содержимое треда RLS
-- блокировал ("Тред недоступен или удалён"), но в "Входящих" остаётся
-- превью-строка с именем собеседника и последним сообщением. Это утечка.
--
-- Добавляем `pt.owner_user_id = p_user_id` — личные диалоги показываются
-- только их владельцу. Менеджерам чужие личные диалоги в инбоксе не нужны,
-- у них есть отдельный UI `/personal-dialogs`.
...
AS $function$
  ...
  accessible_threads AS (
    ...
    UNION ALL
    -- Личные диалоги: только треды текущего юзера.
    SELECT pt.id, pt.project_id, pt.name, pt.icon, pt.accent_color, pt.legacy_channel, pt.access_type, pt.type,
           pt.business_client_tg_user_id, pt.mtproto_client_tg_user_id, pt.wazzup_contact_avatar_url,
           pt.email_last_external_address
    FROM project_threads pt
