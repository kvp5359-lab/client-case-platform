-- Расширяем видимость orphan-задач (project_id=NULL, type='task') ещё одной веткой:
-- участник из project_thread_members ('custom' access). Нужно для UI-фичи
-- «расшарить личную задачу с конкретным сотрудником» (вариант 2 из обсуждения).
...
BEGIN
  ...
  IF v_thread.project_id IS NULL THEN
    ...
    IF v_thread.type = 'task' THEN
      ...
      -- Явно расшаренный участник orphan-задачи (custom access).
      IF EXISTS (
        SELECT 1
...
-- get_workspace_threads: добавляем orphan-задачи, где я в project_thread_members.
