-- Унификация unread/read источников на фронте.
--
-- До: разные части UI читали разные источники:
--   • Бейдж списка «Входящие»  ← inbox v2 RPC (unread_count, manually_unread, ...)
--   • Кнопка «Прочитано/Непрочитано»  ← отдельный RPC get_unread_messages_count
--   • Красный контур у бабблов  ← отдельный select last_read_at из message_read_status
--
-- Из-за этого после reload встречалось расхождение: бейдж/контур говорили
-- «всё прочитано», а кнопка показывала «Прочитано» (т.е. ещё непрочитано).
--
-- После: фронт читает ВСЁ из одной строки inbox v2 RPC. Чтобы это стало
-- возможным, добавляем колонку `last_read_at` в выход RPC. Само значение
-- уже доступно в CTE `manual_unread` (left join по message_read_status),
-- просто не было в SELECT.
