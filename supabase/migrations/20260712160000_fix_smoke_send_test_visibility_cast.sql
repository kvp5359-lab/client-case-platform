-- Фикс: smoke_send_test писал text в enum-колонку project_messages.visibility
-- без каста → INSERT падал «column visibility is of type message_visibility
-- but expression is of type text» (смок красный после деплоя 13 edge-функций
-- 2026-07-12). Реальный мессенджер НЕ затронут — insert из приложения идёт
-- корректным типом; баг только в смок-RPC. Добавлен ::message_visibility.
CREATE OR REPLACE FUNCTION public.smoke_send_test(p_thread_id uuid, p_reply_to uuid DEFAULT NULL::uuid, p_label text DEFAULT NULL::text, p_visibility text DEFAULT 'client'::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (p_thread_id, v_ws, COALESCE(NULLIF(v_name,''),'SMOKE-TEST'), v_pid, 'Владелец', v_body, 'web', COALESCE(p_visibility,'client')::message_visibility, p_reply_to)
