-- Внутренние треды (без подключённых внешних каналов) — это общение между
-- сотрудниками в проекте. Их «отправка» = просто INSERT в БД, никакого
-- внешнего канала не существует.
--
-- BEFORE-INSERT триггер set_initial_send_status ставит pending для всех
-- source='web' (он не знает про каналы — нет thread'ов в контексте при
-- INSERT). Затем dispatch_message_to_channels вызывается через
-- notify_telegram_on_new_message и для внутренних тредов **ничего не
-- делает**, потому что ни одна IF-ветка не срабатывает (нет TG-чата, нет
-- wazzup, нет business, нет mtproto, нет email).
--
-- В итоге сообщение остаётся в pending навсегда. Через 60 сек client-side
-- таймер в DeliveryIndicator красит его в failed → красный бабл с
-- бессмысленной кнопкой «Повторить» (повторять некуда).
--
-- Фикс: в конце dispatch_message_to_channels, если ни одна ветка не
-- сработала, выставляем send_status='sent'. «Нет канала = доставлять
-- нечего = считаем доставленным».
--
-- Плюс одноразовый backfill застрявших сообщений.
