-- Fix: get_inbox_threads_v2 не фильтровал удалённые проекты в CTE accessible_projects.
-- Треды из проектов в корзине (is_deleted = true) могли появляться во входящих.
-- Добавляем AND proj.is_deleted = false.

-- Drop нужен, т.к. CREATE OR REPLACE не позволяет менять body STABLE-функции
-- с тем же return type без DROP (Postgres кеширует plan для STABLE).
