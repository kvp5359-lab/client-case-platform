-- Инбокс: показывать автора события в превью (кто изменил статус и т.п.).
-- Меняем ТОЛЬКО get_inbox_threads_v2 — сигнатура не меняется (last_event_text
-- остаётся text), поэтому обёртки (page/unread/needs/awaiting/search/thread_one)
-- и фронт-типы не трогаются. Имя автора (al.user_id → participants) вшивается
-- в начало last_event_text: «Имя · Статус: …». UI подсвечивает имя статуса по
-- позиции подстроки «Статус: ».
--
-- Тело взято из живой функции прод (репо дрейфует) — изменены только две точки:
--   1) в CTE last_audit добавлен скаляр actor_name;
--   2) last_event_text получает префикс COALESCE(actor_name || ' · ', '').
...
AS $function$
  ...
  last_audit AS (
    SELECT DISTINCT ON (al.resource_id)
      ...
      (SELECT NULLIF(TRIM(COALESCE(pa.name, '') || ' ' || COALESCE(pa.last_name, '')), '')
