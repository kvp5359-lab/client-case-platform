...
describe('sanitizeMessengerHtml — обычные сообщения (без схлопывания)', () => {
  ...
  it('срезает пустоту ПОД последним текстом (хвостовые пустые абзацы)', () => {
    const out = sanitizeMessengerHtml('<p>A</p><p></p><p></p><p><br></p>')
    expect(out).toBe('<p>A</p>')
  })

  it('хвостовые <br> в конце последнего абзаца не оставляют пустого блока', () => {
    const out = sanitizeMessengerHtml('<p>Текст.<br><br></p>')
    expect(out).toBe('<p>Текст.</p>')
  })

  it('внутренние пустые строки при этом сохраняются', () => {
    const out = sanitizeMessengerHtml('<p>A</p><p></p><p>B</p><p></p>')
    expect(out).toBe('<p>A</p><p><br></p><p>B</p>')
  })
