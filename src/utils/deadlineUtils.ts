 ...
 *
 * «Весь день» распознаём по полуночи UTC: дата без времени хранится как
 * `"YYYY-MM-DD"` → БД возвращает её ровно в 00:00:00 UTC (не зависит от таймзоны
 * пользователя). У `WorkspaceTask` нет `start_at`, так что это единственный
 * доступный сигнал. Осознанный край: задача с конкретным временем, которое после
 * конверсии даёт ровно 00:00:00 UTC (напр. 02:00 при UTC+2), сдвинется в конец
 * дня — редко и влияет только на порядок сортировки.
 */
