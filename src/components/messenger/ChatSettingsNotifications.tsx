...
export function ChatSettingsNotifications({
  ...
  // Компактный триггер (рядом с «Подключить канал») → тумблер вкл/выкл.
  if (variant === 'compact') {
    return (
      <button
        type="button"
        ...
        title="Уведомления по этому треду"
        className={cn(
      ...
      >
        ...
        {loading ? 'Загрузка…' : subscribed ? 'Уведомления: вкл' : 'Уведомления: выкл'}
      </button>
    )
  }
