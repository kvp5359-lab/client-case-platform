/**
 * Доменные типы домена «Формы/Анкеты».
 *
 * Нейтральный нижний слой: services/hooks/store зависят сюда, не заходя в UI.
 * Раньше всё жило в `components/forms/types.ts` → инверсия зависимостей
 * (T1 аудита 2026-06-13). `components/forms/types.ts` теперь реэкспортит отсюда.
 */
