      ...
      <div className="h-full overflow-auto bg-gray-50">
        <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-2xl font-semibold">Сводки по проектам</h1>
              <p className="text-sm text-gray-600 mt-1">
                Дневник по всему воркспейсу: выбери дату, посмотри что было в каждом проекте.
                Если сводка не сделана — нажми «Собрать сводку по всем».
              </p>
            </div>
            <div className="flex items-end gap-3">
              <div className="space-y-1">
                <Label htmlFor="ws-digest-date">Дата</Label>
                <Input
                  id="ws-digest-date"
                  type="date"
                  value={date}
            ...
            <CardContent className="py-4 flex flex-wrap items-center gap-3">
              ...
              <Button
                disabled={
                  batchRunning ||
                  loadingProjects ||
                  projectsWithoutDigest.length === 0
                }
                onClick={() =>
                  runBatch(projectsWithoutDigest, { force: false, label: 'Сборка сводок' })
                }
              >
                {batchRunning ? (
                ...
                Собрать сводку по всем ({projectsWithoutDigest.length})
              </Button>
              <Button
                variant="outline"
                disabled={batchRunning || activeProjects.length === 0}
                onClick={() =>
                  runBatch(activeProjects, { force: true, label: 'Перегенерация' })
                }
              >
            ...
            <Card>
              <CardContent className="py-10 text-center text-sm text-gray-600">
                {activeProjects.length === 0
                  ...
                  : 'Сводки на эту дату ещё не собраны. Нажми «Собрать сводку по всем» выше.'}
              </CardContent>
            </Card>
          )}
    ...
    <Card>
      <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
        ...
        <Button variant="ghost" size="sm" onClick={onOpenProject}>
          <ExternalLink className="w-4 h-4 mr-1" /> Открыть проект
        </Button>
      </CardHeader>
      <CardContent>
